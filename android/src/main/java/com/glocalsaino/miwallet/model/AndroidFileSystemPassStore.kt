...
import com.glocalsaino.miwallet.BuildConfig
...
import com.glocalsaino.miwallet.reader.PassReader
import java.io.File
import java.util.*
...
class AndroidFileSystemPassStore(
    ...
    override fun allPasses(): List<Pass> =
        path.listFiles()?.mapNotNull { getPassbookForId(it.name) } ?: emptyList()
    ...
    override fun syncPassStoreWithClassifier(defaultTopic: String) {
        ...
        keysToRemove.forEach { classifier.topicByIdMap.remove(it) }

        var changed = keysToRemove.isNotEmpty()
        path.listFiles()?.forEach {
            if (!classifier.topicByIdMap.containsKey(it.name) && defaultTopic.isNotEmpty()) {
                classifier.topicByIdMap[it.name] = defaultTopic
                changed = true
            }
        }
        if (changed) classifier.processDataChange()
    }
}
