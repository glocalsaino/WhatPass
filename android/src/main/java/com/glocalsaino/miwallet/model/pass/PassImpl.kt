...
class PassImpl(override val id: String) : Pass {
    ...
    @field:[HexColor]
    override var foregroundColor: Int = 0

    @field:[HexColor]
    override var labelColor: Int = 0

    override var transitType: String? = null
    ...
    override var beacons: List<PassBeacon> = ArrayList()

    override var sharingProhibited: Boolean = false
    ...
    override fun getBitmap(passStore: PassStore, @Pass.PassBitmap passBitmap: String): Bitmap? {
        PassBitmapCache.get(id, passBitmap)?.let { return it }
        return try {
            val file = File(passStore.getPathForID(id), passBitmap + FILETYPE_IMAGES)
            BitmapFactory.decodeStream(FileInputStream(file))?.also {
                PassBitmapCache.put(id, passBitmap, it)
            }
