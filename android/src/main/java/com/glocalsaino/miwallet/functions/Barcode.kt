...
fun getBitMatrix(data: String, type: PassBarCodeFormat): BitMatrix {
    // ZXing's default quiet zone is sized for 1D/elongated formats (PDF417, Code 128...)
    // assuming a printed barcode, which leaves a lot of wasted white space when just
    // displayed on screen. A small explicit margin keeps it scannable without that bulk.
    val hints = mapOf(EncodeHintType.MARGIN to 2)
    return MultiFormatWriter().encode(data, type.zxingBarCodeFormat(), 0, 0, hints)!!
}
