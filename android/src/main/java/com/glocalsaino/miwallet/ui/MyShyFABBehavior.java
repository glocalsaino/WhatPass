...
public class MyShyFABBehavior extends CoordinatorLayout.Behavior<FloatingActionsMenu> {
    ...
    // Snackbar.SnackbarLayout is @RestrictedApi (Material's own components are allowed to
    // reference it, external code isn't) - matching by simple class name instead gets the
    // same runtime detection without depending on that restricted type.
    private boolean isSnackbarLayout(View view) {
        return view.getClass().getSimpleName().equals("SnackbarLayout");
    }
